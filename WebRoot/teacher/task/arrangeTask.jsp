...
<link rel="stylesheet" type="text/css" href="../jqueryStep/css/jquery.step.css" />
...
<style>
			...
			.divv1{
			margin-top: 10px;margin-left: 10px;float: left;border: 1px solid #dcdcdc;border-radius: 5px;position: relative;
			}
			.pp1{
			padding-left: 8px;padding-right: 30px;line-height: 30px;white-space: nowrap;
			}
			.pp2{
			width:30px;line-height: 30px;height: 30px;text-align: center;position: absolute;right: 0px;top: 0;cursor: pointer;
			}
		</style>
</head>
<body>
		...
		<div class=" admin-content">
			...
			<div class="admin-biaogelist" style="positioin:absolute;top:30px;">
				...
				<div class="fbneirong">
					...
					<form class="am-form" id="form" action="Task-arrangeTask.action" enctype="multipart/form-data" method="post" onsubmit="return verify()">
						<div id="corpus">
							<div style="display:inline-block;width:710px;">
								<div class="am-form-group am-cf">
									<div class="zuo">作业名称：</div>
									<div class="you" style="width:300px;">
										<input type="text" class="am-input-sm" id="taskname" name="tp.task_name"
											placeholder="请输入标题" required>
									</div>
									<div class="you" style="width:5px;">
										&nbsp;
									</div>
									<div class="you" style="width:300px;">
										<button type="button" class="am-btn am-btn-primary am-round" id="oldTask" data-am-modal="{target: '#my-popup'}">选择作业</button>
									</div>
								</div>
								<input type="hidden" value='<s:property value="#session['teacher'].tname"/>' name="tp.tname">
								<div class="am-form-group am-cf">
								...
								<div class="am-form-group am-cf">
									...
									<div class="you">
										<div style="background: #f4f4f4;background-color: red" id="selectedChapter">
											<div class="divv1">
											    <p class="pp1">第一章 第一节</p>
												<p class="pp2">
													<span onClick="deleteChapter()" id="dealChapter"><i style="background-position: -18px -430px;display: block;">&times;</i></span>
												</p>
											</div>
											...
											<div class="divv1">
												<p class="pp1"> 第一章</p>
												<p class="pp2">
													<span><i style="background-position: -18px -430px;display: block;">&times;</i></span>
												</p>
											</div>
											<div class="divv1">
												<p class="pp1"> 第一章</p>
										...
										<input type="text" class="am-input-sm" id="taskchapter" name="tp.tchapter" value="第一章"
											placeholder="请选择开始章" onClick="chooseChapter()" required>
									</div>
								</div>
								<input type="hidden" value="<s:property value="#session['teacher'].tno"/>" name="tp.tno">
								<input type="hidden" value="<s:property value="#session['teacher'].tname"/>" name="tp.tname">
								<div class="am-form-group am-cf">
							...
							<div style="display:inline;background-color: blue">
								章节
								<select data-am-selected="{btnWidth: 190, btnSize: 'sm', btnStyle: 'default'}" multiple="multiple" size="6">
							   ...
							   <div class="div4">
									<dl class="dl1">
										<dd class="dd1" id="ddbutton">
											<input type="button" id="tixing_danxuan" class="am-btn am-btn-primary input1" value="单选题">
											<input type="button" id="tixing_duoxuan" class="am-btn am-btn-primary input1" value="多选题">
											<input type="button" id="tixing_tiankong" class="am-btn am-btn-primary input1" value="填空题">
											<input type="button" id="tixing_panduan" class="am-btn am-btn-primary input1" value="判断题">
											<input type="button" id="tixing_jianda" class="am-btn am-btn-primary input1" value="简答题">
											<input type="button" id="tixing_daima" class="am-btn am-btn-primary input1" value="编程题">
										</dd>
									</dl>
								</div>
						...
						<div id="class" style="display:none;min-height: 200px;max-width: 600px;margin: 0 auto">
	...
	<div class="am-popup" id="my-popup" style="width:600px;margin-left:400px;heigth:200px;margin-top: 20px;margin-bottom: 20px">
	...
	<script type="text/javascript">
		...
		var score=0;
		$( function (){
			var str=new Array("单选题","多选题","填空题","判断题","简答题","代码题");
            $( "input[id^='tixing_']" ).each( function(){//获取所有的id为tochannel_开头的Button  
                $( this ).click(function(){//绑定当前点击的按钮  
                    var ids = $( this).val();//获取它的id属性值 
                    console.log(ids);
                    var cname=document.getElementById("course").value;//获取输入的课程名 
                    console.log(cname);
                    var chapter=document.getElementById("taskchapter").value;//获取输入的章节
                    console.log(chapter);
                    score++;
                    var index=0;
                    ...
                    $.ajax({
        				...
        				data : {"type":ids,"cno":cname,"chapter":chapter},
        				dataType : "json",
        				success : function(data) {
        					...
        					$("#user_ul").append("<li class='li1' id="+ids+">"
	    		                    +"<b class='b1'>"+num+"</b>"
	    		+"<span class='span1'>"+ids+"</span>"
	    		+"<div class='div2'>"
	    		+"<input type='text' onChange='totalscore1(this,"+num+")' onkeypress='keyPress(this)' onkeyup='keyUp(this)' onblur='onBlur(this)' name='ampp.count' value='1' style='height: 18px;border: 1px solid #dcdcdc;padding: 3px;width: 50px;margin-right: 5px;text-align: center;float: left;margin-top: 10px;color: #1b92e2;font-size: 14px;font-weight: 700;'>题"
	    		+"</div>"
	    		...
	    		+"</div>"
	    		+"<a class='a1' onClick='deletetixing(this)' name="+ids+"><i class='am-icon-trash-o' style='display: inline-block;margin-left: 16px;margin-top: -10px;height: 12px;width: 12px;'></i></a>"
	    	    +"</li>");
        				}
        			});
		...
		function deletetixing(obj){
			...
			$("#"+tixing).remove();
			$("#ddbutton").find("input[type='button'][value="+tixing+"]").removeAttr("disabled");
			score--;
			...
			$.ajax({
				...
				success : function(data) {
					...
					for(var i=0;i<d.task.length;i++){
						html="<tr>"+ 
							"<td>"+d.task[i].task_name+"</td>"+
						      "<td>"+d.task[i].cname+"</td>"+
						      "<td>"+d.task[i].cname+"</td>"+
						      "<td>"+d.task[i].tchapter+"</td>"+
						      "<td><button type='button' onClick='selectedTask("+i+")'>选择</button></td>"+
						      "</tr>";
